...
async function jupyterConfigData() {
  ...
  // Each workspace gets its own IndexedDB partition so users can't see each other's files.
  // The 'files' store is where JupyterLite saves all file content.
  // We back that store up to the server via the /workspaces/sync endpoint and restore it on login.
  try {
    const urlParams = new URLSearchParams(window.location.search);
    const ws = urlParams.get('workspace');
    if (ws) {
      // Scope all three storage namespaces so each workspace is fully isolated
      finalConfig.contentsStorageName  = `JupyterLite Storage - ${ws}`;
      finalConfig.settingsStorageName  = `JupyterLite Settings - ${ws}`;
      finalConfig.workspacesStorageName = `JupyterLite Workspaces - ${ws}`;
      console.log(`[DEP Workspace] Isolated to: "${ws}"`);
    }
  } catch (e) {
    console.error('Error applying workspace isolation:', e);
  }
